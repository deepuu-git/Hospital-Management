...
// to getAappointments
export const getAappointments = async (req, res) => {
  try {
    const {
      doctorId,
      mobile,
...
export const getAappointmentsByPatient = async (req, res) => {
  try {
...
export const createAppointment = async (req, res) => {
  try {
    ...
    let resolvesOwner = ownerFromBody || doctor.owner || null;
    if (!resolvesOwner) resolvesOwner = MAJOR_ADMIN_ID || String(doctorId);
    ...
    try {
      ...
      return res.status(502).json({
        success: false,
        message: `payment provide error: ${message}`,
      });
    }
        ...
        appt = await Appointment.findOneAndUpdate(
          {
            ...
            fees: Math.round((session.amount_total || 0) / 100),
          },
          {
            "payment.status": "paid",
...
export const updateAppointment = async (req, res) => {
  try {
    ...
    if (terminal && body.status && body.status !== appt.status) {
      return res.status(400).json({
        success: false,
        message: "Cannot change status of completed/canceled appointment",
      });
    }
    ...
    if (body.date && body.time) {
      ...
      update.rescheduleTo = { date: body.date, time: body.time };
    }
    ...
    return res.json({
      success: true,
      appointment: updated, // ✅ FIX (you were returning wrong variable)
    });
  } catch (error) {
    console.error("updateAppointment error:", error); // ✅ FIX (err -> error)
    return res.status(500).json({
      success: false,
      message: "Server error",
    });
...
// to getAppoinements by Doctor
export const getAappointmentsByDoctor = async (req, res) => {
  try {
    ...
    console.error("getAappointmentsByDoctor error:", error);
    return res.status(500).json({
      success: false,
      message: "Server error",
    });
