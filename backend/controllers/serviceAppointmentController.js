...
// this function will getIther user from clerk and return the user details
...
export const createServiceAppointment = async (req, res) => {
  try {
    ...
    if (numericAmount === null || numericAmount < 0)
      return res
        .status(400)
        .json({ success: false, message: "amount must be valid" });
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          "Time missing or invalid - provide hour/minute/ampm or valid time string ",
      });
    }
    ...
    try {
      ...
      if (existing)
        return res.status(409).json({
          success: false,
          message: "You already have a  booking at this time for this service.",
        });
    } catch (chkErr) {
      console.warn("Duplicate booking failed:", chkErr);
    }
    ...
    const svcImageUrlFromDB =
      svc && String(svc.imageUrl || svc.image || svc.image?.url || "");

    const svcImagePublicIdFromDB =
      svc && String(svc.imagePublicId || svc.image?.publicId || "");

    const finalServiceImageUrl =
      svcImageUrlFromDB && svcImageUrlFromDB.length ? svcImageUrlFromDB : "";
    ...
    if (!frontendBase)
      return res.status(500).json({
        success: false,
        message:
          "Frontend base URL not availale. Set FRONTEND_URL or provide Origin header.",
      });
    ...
    try {
        ...
        line_items: [
          {
            price_data: {
              ...
              product_data: {
                ...
                description: `Appointment on ${base.date} ${base.hour}:${String(base.minute || "00")}`,
              },
      ...
      return res.status(502).json({
        success: false,
        message: `Payment provider error: ${message || "Unknown error"}`,
      });
    }
...
export const confirmServicePayment = async (req, res) => {
  try {
    ...
    if (!appt && session.metadata?.appointment) {
      appt = await ServiceAppointment.findOneAndUpdate(
        { _id: session.metadata.appointment },
        {
          $set: {
...
export const updateServiceAppointment = async (req, res) => {
  try {
    ...
    // first check whether fill if yes then update  the file
...
export const getServiceAppointmentStats = async (req, res) => {
  try {
    const services = await Service.aggregate([
      ...
      { $addFields: { earing: { $multiply: ["$completed", "$price"] } } },
      {
        $project: {
          name: 1,
          price: 1,
