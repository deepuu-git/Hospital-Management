...
const fee = safeNumber(
  doc.fee ?? doc.fees ?? doc.consultationFee ?? doc.consultation_fee ?? 0, 0,
);
const image = 
doc.imageUrl || doc.image || doc.avatar || `https://i.pravatar.cc/150?u=${id}`;
...
const DashboardPage = () => {
  ...
  // new pateint count
  const [pateintCount, setPatientCount] = useState(null);
  const [patientsCountLoading, setPatientCountLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    async function loadDoctors() {
      ...
      try {
       ...
       let list = []; 
       if (Array.isArray(body)) list = body; else if (Array.isArray(body.doctors)) list = body.doctors;
       else if (Array.isArray(body.data)) list = body.data; 
       else if (Array.isArray(body.items)) list = body.items; 
       else { 
        const firstArray = Object.values(body).find((v) => Array.isArray(v)); 
        if (firstArray) list = firstArray; } const normalized = list.map((d) => normalizeDoctor(d)); 

        if (mounted) setDoctors(normalized); } catch (err) { console.error("Failed to load doctors:", err);
           if (mounted) { setError(err.message || "Failed to load doctors"); 
            setDoctors([]); } } finally { if (mounted) setLoading(false); } } loadDoctors(); return () => { mounted = false; }; }, []);
  ...
  const totals = useMemo(() => {
    ...
    const totalAppointments = doctors.reduce(
      (s, d) => s + safeNumber(d.appointments?.total, 0), 0,
    );
    const totalEarnings = doctors.reduce(
      (s, d) => s + safeNumber(d.earnings, 0), 0,
    );
    const completed = doctors.reduce(
      (s, d) => s + safeNumber(d.appointments?.completed, 0), 0,
    );
    const canceled = doctors.reduce(
      (s, d) => s + safeNumber(d.appointments?.canceled, 0), 0,
    );
    const totalLoginPatients = doctors.reduce((s, d) => s + (d.raw?.loginPatientsCount ?? 0), 0) || 0;
    return {
      totalDoctors,
      totalAppointments,
      totalEarnings,
         ...
         label="Total Registered Users"
          value={
            patientsCountLoading
             ? "Loading..."
             : (pateintCount ?? totals.totalLoginPatients)
          } />

           <StatCard 
           icon={<CalendarRange className= "w-6 h-6" />}
         label="Total Appoiintments" value={totals.totalAppointments} />
            ...
            <p className={s.tableCount}>
              ...
              {filteredDoctors.length > INITIAL_COUNT && (
                <div className={s.showMoreContainer}>
                  <butto onClick={() => setShowAll((s) => !s)} className={s.showMoreButton + " " + s.cursorPointer}>
                     {showAll ? 
