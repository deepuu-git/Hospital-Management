...
import { DollarSign, EyeClosed, Search, Star, Users, Trash2, BadgeIndianRupee } from 'lucide-react';
...
// it will normalize any date -like sstring
...
// that is nearest date comes firas;
function getSortedScheduleDates(schdeuleLike) {
    let keys = [];
    if (Array.isArray(schdeuleLike)) {
        keys = schdeuleLike.map(normalizeToDateString).filter(Boolean);      
    } else if (schdeuleLike && typeof schdeuleLike === "object") {
        keys = Object.keys(schdeuleLike).map(normalizeToDateString).filter(Boolean);
    }
  ...
  return (
    <div className={doctorListStyles.container}>
        ...
        <main className={doctorListStyles.gridContainer}>
            {loading && (
                <div className={doctorListStyles.loadingContainer}>Loading Doctors...</div>
            )}
            ...
            {displayed.map((doc) => {
                ...
                return (
                    ...
                    <div className={doctorListStyles.articleContent}>
                        ...
                        <img src={doc.imageUrl || doc.image || null} alt={doc.name} className={doctorListStyles.doctorImage} />
                        ...
                        style={{
                            ...
                            transition: "max-height 420ms cubic-bezier(.2,.9,.2,1), padding 220ms ease",
                        ...
                        {isOpen && (
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                <div className={doctorListStyles.aboutSection}>
                                    ...
                                    <div className='mt-4'>
                                       <div className={doctorListStyles.qualificationsHeading}>Qualificatioin</div>
                                       <div className={DetailsSharp.qualificationsText}>{doc.qualifications}</div>
                                    </div>

                                    <div className='mt-4'>
                                        <div className={doctorListStyles.scheduleHeading}>Schedule</div>

                                        <div className='mt-2 flexflex-wrap gap-2'>{sortedDates.map((date) => {
                                            const slots = scheduleMap[date] || [];
                                            return (
                                                <div  key={date}className='min-w-full md:min-w-0'>
                                                    <div className={doctorListStyles.scheduleDate}>{formatDateISO(date)}</div>
                                                    <div className='mt-1 flex flex-wrap gap-2'>{slots.map((s, i) => (
                                                        <span key={i} className={doctorListStyles.scheduleSlot}>{s}
                                                        </span>
                                                    ))}
                                                    </div>
                                                </div>
                                ...
                                <aside className="flex flex-wrap gap-x-6 gap-y-2 mt-2">
                                    
                                    <div className={doctorListStyles.scheduleHeading}>Success</div>

                                    <div className={doctorListStyles.statsItemValue}>{doc.success}%</div>

                                    <div className={doctorListStyles.statsItemHeading}>Patients</div>

                                    <div className={doctorListStyles.statsItemValue}>{doc.patients}</div>

                                    <div className={doctorListStyles.statsItemHeading}>Location</div>

                                    <div className={doctorListStyles.locationValue}>{doc.location}</div>
                ...
                {filtered.length > 6 && (
                    <div className={doctorListStyles.showMoreContainer}>
                        <button onClick={() => setShowAll((s) => !s)} className={doctorListStyles.showMoreButton}>{showAll ? "Show Less" : `Show more (${filtered.length - 4})`}
                        </button>
                    </div>
                )}
        </main>
