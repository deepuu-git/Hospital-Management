...
const DEFAULT_HOST = "https://backend-wy5h.onrender.com".replace(/\/$/, "");
...
export default function ServiceDetail() {
  ...
  function transformServiceShape(doc) {
    ...
    if (
      ...
      console.log("RAW SLOTS:", doc.slots);
      console.log("OBJECT SLOT DETECTED");

      Object.entries(doc.slots).forEach(([dateKey, times]) => {
        console.log("DATE KEY:", dateKey);

        const normalizedDate = normalizeToDateString(dateKey);

        console.log("NORMALIZED:", normalizedDate);

        if (normalizedDate) {
          slotsMap[normalizedDate] = Array.isArray(times) ? times : [];
        }
      });

      dates = Object.keys(slotsMap);

      console.log("FINAL DATES:", dates);
      console.log("FINAL SLOTSMAP:", slotsMap);
